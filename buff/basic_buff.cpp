...
inline void Detector::getInput(cv::Mat& _input_img, const int& _my_color) {
  src_img_ = _input_img;
  my_color_ = _my_color;
  src_img_.copyTo(dst_img_);  // 复制当前帧图像
  is_find_target_ = false;    //
}
...
void Detector::runTask(cv::Mat& _input_img,
  ...
  // 计算预测量 单位为弧度
  final_forecast_quantity_ = doPredict(
      static_cast<float>(_receive_info.bullet_velocity), is_find_target_);

  // 计算获取最终目标（矩形、顶点）
  calculateTargetPointSet(final_forecast_quantity_, final_center_r_,
                          target_2d_point_, dst_img_, is_find_target_);
  ...
  if (is_find_target_) {
    ...
    fmt::print("[{}] Info, yaw: {}, pitch: {}, depth: {}\n", idntifier_yellow,
               _send_info.yaw_angle, _send_info.pitch_angle, _send_info.depth);
  } else {
...
uart::Write_Data Detector::runTask(cv::Mat& _input_img,
  ...
  if (is_find_target_) {
    ...
    send_info.yaw_angle =
        angleCalculation(pre_center_, 0.0048, src_img_.size(), 8).x;
    send_info.pitch_angle =
        angleCalculation(pre_center_, 0.0048, src_img_.size(), 8).y;
    cv::Point yaw_angle = cv::Point(dst_img_.cols - 100, 60);
    cv::putText(dst_img_, std::to_string(send_info.yaw_angle), yaw_angle,
                cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 0), 1, 8,
                false);
    cv::Point pitch_angle = cv::Point(dst_img_.cols - 100, 70);
    cv::putText(dst_img_, std::to_string(send_info.pitch_angle), pitch_angle,
                cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(25, 255, 255), 1, 8,
                false);
    ...
    fmt::print("[{}] Info, yaw: {}, pitch: {}, depth: {}\n", idntifier_yellow,
               send_info.yaw_angle, send_info.pitch_angle, send_info.depth);
  } else {
...
void Detector::readBuffConfig(const cv::FileStorage& _fs) {
  ...
  _fs["CENTER_R_ASPECT_RATIO_MIN"] >>
      buff_config_.param.CENTER_R_ASPECT_RATIO_MIN;
  _fs["CENTER_R_ASPECT_RATIO_MAX"] >>
      buff_config_.param.CENTER_R_ASPECT_RATIO_MAX;
  ...
  // 手算pitch 轴弹道补偿
  _fs["OFFSET_MANUAL_ARMOR_PITCH"] >>
      buff_config_.param.OFFSET_MANUAL_ARMOR_PITCH;
  buff_config_.param.OFFSET_MANUAL_ARMOR_PITCH *= 0.01;

  // 输出提示
  fmt::print(
      "✔️ ✔️ ✔️ 🌈 能量机关初始化参数 读取成功 🌈 "
      "✔️ "
      "✔️ "
      "✔️\n");
}
...
// ============ 预处理 ============ //
void Detector::imageProcessing(cv::Mat& _input_img, cv::Mat& _output_img,
                               const int& _my_color,
...
// 显示各部分的二值图
  ...
  if (buff_config_.ctrl.IS_SHOW_BIN_IMG == 1 &&
    ...
    cv::imshow("[basic_buff] imageProcessing() -> bin_img_color_",
               bin_img_color_);
    cv::imshow("[basic_buff] imageProcessing() -> bin_img_gray_",
               bin_img_gray_);
  }
#endif  // !RELEASE
...
void Detector::bgrProcessing(const int& _my_color) {
  ...
  switch (_my_color) {
    case uart::RED: {
      ...
      if (buff_config_.ctrl.IS_PARAM_ADJUSTMENT == 1) {
        ...
        cv::createTrackbar("GRAY_TH_RED:", window_name,
                           &buff_config_.param.RED_BUFF_GRAY_TH, 255, nullptr);
        cv::createTrackbar("COLOR_TH_RED:", window_name,
                           &buff_config_.param.RED_BUFF_COLOR_TH, 255, nullptr);
      ...
      // 亮度部分
      cv::threshold(gray_img_, bin_img_gray_,
                    buff_config_.param.RED_BUFF_GRAY_TH, 255,
                    cv::THRESH_BINARY);

      // 颜色部分
      cv::threshold(bin_img_color_, bin_img_color_,
                    buff_config_.param.RED_BUFF_COLOR_TH, 255,
                    cv::THRESH_BINARY);
    ...
    case uart::BLUE: {
      ...
      if (buff_config_.ctrl.IS_PARAM_ADJUSTMENT == 1) {
        ...
        cv::namedWindow(window_name);
        cv::createTrackbar("GRAY_TH_BLUE:", window_name,
                           &buff_config_.param.BLUE_BUFF_GRAY_TH, 255, nullptr);
        cv::createTrackbar("COLOR_TH_BLUE:", window_name,
                           &buff_config_.param.BLUE_BUFF_COLOR_TH, 255,
                           nullptr);
      ...
      // 亮度部分
      cv::threshold(gray_img_, bin_img_gray_,
                    buff_config_.param.BLUE_BUFF_GRAY_TH, 255,
                    cv::THRESH_BINARY);
    ...
    default: {
      ...
      // 亮度部分
      average_th_ = static_cast<int>((buff_config_.param.RED_BUFF_GRAY_TH +
                                      buff_config_.param.BLUE_BUFF_GRAY_TH) *
                                     0.5);
      cv::threshold(gray_img_, bin_img_gray_, average_th_, 255,
                    cv::THRESH_BINARY);

      // 颜色部分
      cv::threshold(bin_img_color1_, bin_img_color1_,
                    buff_config_.param.BLUE_BUFF_COLOR_TH, 255,
                    cv::THRESH_BINARY);
      cv::threshold(bin_img_color2_, bin_img_color2_,
                    buff_config_.param.RED_BUFF_COLOR_TH, 255,
                    cv::THRESH_BINARY);
  ...
  std::vector<cv::Mat>(split_img_)
      .swap(split_img_);  // TODO(fqjun) :查看容量有多大
}
  ...
  switch (_my_color) {
    case uart::RED:
      ...
      if (buff_config_.ctrl.IS_PARAM_ADJUSTMENT == 1) {
        ...
        cv::createTrackbar("GRAY_TH_RED:", window_name,
                           &buff_config_.param.RED_BUFF_GRAY_TH, 255, nullptr);
        cv::createTrackbar("H_RED_MAX:", window_name,
                           &buff_config_.param.H_RED_MAX, 360, nullptr);
        cv::createTrackbar("H_RED_MIN:", window_name,
                           &buff_config_.param.H_RED_MIN, 360, nullptr);
        cv::createTrackbar("S_RED_MAX:", window_name,
                           &buff_config_.param.S_RED_MAX, 255, nullptr);
        cv::createTrackbar("S_RED_MIN:", window_name,
                           &buff_config_.param.S_RED_MIN, 255, nullptr);
        cv::createTrackbar("V_RED_MAX:", window_name,
                           &buff_config_.param.V_RED_MAX, 255, nullptr);
        cv::createTrackbar("V_RED_MIN:", window_name,
                           &buff_config_.param.V_RED_MIN, 255, nullptr);
    ...
    case uart::BLUE:
      ...
      if (buff_config_.ctrl.IS_PARAM_ADJUSTMENT == 1) {
        ...
        cv::createTrackbar("GRAY_TH_BLUE:", window_name,
                           &buff_config_.param.BLUE_BUFF_GRAY_TH, 255, nullptr);
        cv::createTrackbar("H_BLUE_MAX:", window_name,
                           &buff_config_.param.H_BLUE_MAX, 255, nullptr);
        cv::createTrackbar("H_BLUE_MIN:", window_name,
                           &buff_config_.param.H_BLUE_MIN, 255, nullptr);
        cv::createTrackbar("S_BLUE_MAX:", window_name,
                           &buff_config_.param.S_BLUE_MAX, 255, nullptr);
        cv::createTrackbar("S_BLUE_MIN:", window_name,
                           &buff_config_.param.S_BLUE_MIN, 255, nullptr);
        cv::createTrackbar("V_BLUE_MAX:", window_name,
                           &buff_config_.param.V_BLUE_MAX, 255, nullptr);
        cv::createTrackbar("V_BLUE_MIN:", window_name,
                           &buff_config_.param.V_BLUE_MIN, 255, nullptr);
...
void Detector::findTarget(cv::Mat& _input_dst_img, cv::Mat& _input_bin_img,
  ...
  for (size_t i = 0; i != contours_.size(); ++i) {
    ...
    // 大轮廓面积条件
    big_rect_area_ =
        cv::contourArea(contours_[static_cast<uint>(hierarchy_[i][3])]);
...
cv::Point2f Detector::findCircleR(cv::Mat& _input_src_img,
                                  cv::Mat& _input_bin_img, cv::Mat& _dst_img,
  ...
  roi_global_center_ = current_target_.Armor().Rect().center -
  ...
  // 查找轮廓
  cv::findContours(result_img_, contours_r_, cv::RETR_EXTERNAL,
                   cv::CHAIN_APPROX_NONE);
  ...
  for (size_t i = 0; i != contours_r_.size(); ++i) {
    ...
    if (center_r_.Rect().boundingRect().area() <
            buff_config_.param.CENTER_R_AREA_MIN ||
        center_r_.Rect().boundingRect().area() >
            buff_config_.param.CENTER_R_AREA_MAX) {
      continue;
    }

    fmt::print("[{}] Find center R target success !!!   ", center_yellow);
    fmt::print(" --》 矩形 {}  --》 Ratio: {} / Area: {} ", i,
               center_r_.aspectRatio(), center_r_.Rect().boundingRect().area());
  ...
  if (center_r_box_.size() < 1) {
    ...
    std::sort(
        center_r_box_.begin(), center_r_box_.end(),
    ...
    is_circle_ = true;
    center_r_point2f =
        center_r_box_[0].Rect().center + roi_R.boundingRect2f().tl();
...
void Detector::calAngle() {
  ...
  // TODO(fqjun) :当变化量大于 30°时，则是切换装甲板，则重置 diff 为 0，last
  // 为当前。
}
...
void Detector::calDirection() {
  ...
  if (find_cnt_ % 2 == 0) {
    ...
    filter_direction_ =
        (1 - 0.01) * last_direction_ + 0.01 * current_direction_;
...
void Detector::calVelocity() {
  ...
  float current_diff_angle =
      diff_angle_ + last_diff_angle_ + last_last_diff_angle_;
  // 默认单位为角度/s
  if (current_time == 0) {
    current_speed_ = 0.f;
  } else {
...
float Detector::doPredict(const float& _bullet_velocity,
                          const bool& _is_find_target) {
#ifdef DEBUG_KALMAN
#ifndef RELEASE
  ...
  // 计算固定预测量 原来是给0.35弧度 TODO(fqjun)
  // :测一下最快和最慢速度时的提前量，以确定范围 predict_quantity =
...
float Detector::fixedPredict(const float& _bullet_velocity) {
  ...
  // 计算子弹飞行时间
  bullet_tof_ =
      (target_z_ + buff_config_.param.OFFSET_TARGET_Z) / _bullet_velocity;
...
void Detector::calculateTargetPointSet(
    const float& _predict_quantity, const cv::Point2f& _final_center_r,
  ...
  // 计算最终坐标点
  radio_ = abstract_object::centerDistance(
      _final_center_r, current_target_.Armor().Rect().center);
  pre_center_.x = radio_ * cos_calcu_ + _final_center_r.x;
  pre_center_.y = radio_ * sin_calcu_ + _final_center_r.y;

  // 计算最终目标的旋转矩形
  target_rect_ =
      cv::RotatedRect(pre_center_, current_target_.Armor().Rect().size, 90);
  ...
  cv::circle(_input_dst_img, _final_center_r, radio_, cv::Scalar(0, 255, 125),
             2, 8, 0);  // 轨迹圆
  cv::circle(_input_dst_img, pre_center_, 3, cv::Scalar(255, 0, 0), 3, 8,
             0);  // 预测值的中点
  cv::line(_input_dst_img, pre_center_, _final_center_r,
           cv::Scalar(0, 255, 255), 2);  // 预测点和圆心的连线
  cv::line(_input_dst_img, current_target_.Armor().Rect().center,
           _final_center_r, cv::Scalar(0, 255, 0), 2);  // 装甲板和圆心的连线

  // 顺时针表示顶点顺序,红黄蓝绿
  cv::circle(_input_dst_img, _target_2d_point[0], 10, cv::Scalar(0, 0, 255), -1,
             8, 0);
  cv::circle(_input_dst_img, _target_2d_point[1], 10, cv::Scalar(0, 255, 255),
             -1, 8, 0);
  cv::circle(_input_dst_img, _target_2d_point[2], 10, cv::Scalar(255, 0, 0), -1,
             8, 0);
  cv::circle(_input_dst_img, _target_2d_point[3], 10, cv::Scalar(0, 255, 0), -1,
             8, 0);
  // 绘制图像
#endif  // RELEASE
}
...
cv::Point2f Detector::angleCalculation(const cv::Point2f& _target_center,
                                       const float& _unit_pixel_length,
                                       const cv::Size& _image_size,
                                       const float& _focal_length) {
  cv::Point2f angle2f;

  float target_projection_x =
      fabs(_image_size.width * 0.5 - _target_center.x) * _unit_pixel_length;
