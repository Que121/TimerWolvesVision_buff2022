...
SerialPort::SerialPort(std::string _serial_config) {
  ...
  opt.c_lflag &=
      ~(ICANON | ECHO | ECHOE |
        ISIG);  // ICANON启用标准模式;ECHO回显输入字符;ECHOE如果同时设置了
                // ICANON，字符 ERASE 擦除前一个输入字符，WERASE
                // 擦除前一个词;ISIG当接受到字符 INTR, QUIT, SUSP, 或
                // DSUSP 时，产生相应的信号
  ...
  opt.c_oflag &= ~(
      ONLCR |
      OCRNL);  // ONLCR将输出中的新行符映射为回车-换行,OCRNL将输出中的回车映射为新行符

  /* c_iflag 输入模式 */
  opt.c_iflag &= ~(ICRNL | INLCR);  // ICRNL将输入中的回车翻译为新行 (除非设置了
                                    // IGNCR),INLCR将输入中的 NL 翻译为 CR
  opt.c_iflag &=
      ~(IXON | IXOFF | IXANY);  // IXON启用输出的 XON/XOFF流控制,IXOFF启用输入的
                                // XON/XOFF流控制,IXANY(不属于 POSIX.1；XSI)
                                // 允许任何字符来重新开始输出
  ...
  opt.c_cflag &=
      ~CSTOPB;  // CSTOPB设置两个停止位，而不是一个,加~就是设置一个停止位
  opt.c_cflag &=
      ~PARENB;  // PARENB允许输出产生奇偶信息以及输入的奇偶校验,加~就是无校验
  ...
  /* new_cfg.c_cc[VMIN]   =   8;//DATA_LEN;
     ...
     另外特别注意的是当设置VTIME后，如果read第三个参数小于VMIN ，将会将VMIN
     修改为read的第三个参数*/
  ...
  tcsetattr(fd, TCSANOW,
            &opt);  //设置终端控制属性,TCSANOW：不等数据传输完毕就立即改变属性
}
//======= 配置串口 =======//
...
void SerialPort::writeData(const int& _yaw, const int16_t& yaw,
  ...
  if (serial_config_.show_serial_information == 1) {
    ...
    // static_cast<int>(write_buff_[6]), static_cast<float>(pitch_reduction_) /
    // 100, static_cast<float>(depth_reduction_)); for (size_t i = 11; i != 12;
    // ++i) {
    //   fmt::print(" {}", write_buff_[i]);
    // }
    // fmt::print("\n");
...
void SerialPort::writeData(const Write_Data& _write_data) {
  ...
  writeData(write_data_.symbol_yaw, write_data_.yaw_angle,
            ...
            write_data_.depth, write_data_.data_type, write_data_.is_shooting);
}
...
//   writeData(write_data_.symbol_yaw, write_data_.yaw_angle,
//   write_data_.symbol_pitch, write_data_.pitch_angle, write_data_.depth,
//   write_data_.data_type, write_data_.is_shooting);
// }
...
void SerialPort::getDataForSend(const int& data_type, const int& is_shooting,
  ...
  write_buff_[0] = 0x53;
  write_buff_[1] =
      static_cast<unsigned char>(data_type);  // 是否发现目标  1为发现 0为未发现
  write_buff_[2] = static_cast<unsigned char>(
      is_shooting);  // 是否射击   1为射击 0为射击 单发
  ...
  write_buff_[9] =
      returnLowBit(depth);  // 最终打击目标的深度信息（预测点） 低八位
  write_buff_[10] =
      returnHighBit(depth);  // 最终打击目标的深度信息（预测点） 高八位
...
void SerialPort::updateReceiveInformation() {
  ...
  for (size_t i = 0; i != sizeof(transform_arr_) / sizeof(transform_arr_[0]);
       ++i) {
    transform_arr_[i] =
        receive_buff_[i + 1];  // - '0' 删除  2022/7/8 华哥删的 自己悟
  ...
  //===========陀螺仪Angle数据===========//
  for (size_t i = 0;
       i != sizeof(receive_data_.Receive_Yaw_Angle_Info.arr_yaw_angle); ++i) {
    receive_data_.Receive_Yaw_Angle_Info.arr_yaw_angle[i] =
        receive_buff_[i + 4];
  ...
  for (size_t i = 0;
       i !=
       sizeof(this->receive_data_.Receive_Pitch_Angle_Info.arr_pitch_angle);
       ++i) {
    receive_data_.Receive_Pitch_Angle_Info.arr_pitch_angle[i] =
        receive_buff_[i + 8];
  ...
  // for (size_t i = 0; i !=
  // sizeof(receive_data_.Receive_Yaw_Velocity_Info.arr_yaw_velocity); ++i) {
  //   receive_data_.Receive_Yaw_Velocity_Info.arr_yaw_velocity[i] =
  //   receive_buff_[i + 10];
  // }

  // for (size_t i = 0; i !=
  // sizeof(this->receive_data_.Receive_Pitch_Velocity_Info.arr_pitch_velocity);
  // ++i) {
  //   receive_data_.Receive_Pitch_Velocity_Info.arr_pitch_velocity[i] =
  //   receive_buff_[i + 12];
  // }
}
}  // namespace uart