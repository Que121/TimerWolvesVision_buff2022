...
// fmt打印信息
auto idntifier_green =
    fmt::format(fg(fmt::color::green) | fmt::emphasis::bold, "uart_serial");
auto idntifier_red =
    fmt::format(fg(fmt::color::red) | fmt::emphasis::bold, "uart_serial");
...
class SerialPort {
 public:
   ...
   * @return float
   */
  inline float returnReceivePitchVelocity() {
    return receive_data_.pitch_veloctiy;
  }
   ...
   */
  void writeData(const int& _yaw, const int16_t& yaw, const int& _pitch,
